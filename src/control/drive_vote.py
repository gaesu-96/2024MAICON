...
from detection_vote import myYOLO
...
class MaiconBot:
    def __init__(self):
        ...
        self.controller = myYOLO("A.pt")
        self.controller2 = myYOLO("B.pt")
        self.controller3 = myYOLO("C.pt")
    ...
    @staticmethod
    def voting_on_detections(voter1, voter2, voter3, criterion="democracy"):
        def _mode(data1, data2, data3):
        ...
        final_vote = {"AF": 0, "EF": 0, "AT": 0, "ET": 0}
        for k in final_vote.keys():
            if 'T' in k:
                final_vote[k] = voter1[k]
                continue
            print(f'[+] Criterion({criterion} {k}) : {voter1[k]}(*) {voter2[k]} {voter3[k]}')
            if criterion == "democracy":
                final_vote[k] = _mode(voter1[k], voter2[k], voter3[k])
            else:
                final_vote[k] = voter1[k]
    ...
    def detection(self, stage = 'A'):
        ...
        for idx in range(1):
            ...
            # print(f'[+] Detected Result #{idx+1} : AF-{detection_result["AF"]} / EF-{detection_result["EF"]} / AT-{detection_result["AT"]} / ET-{detection_result["ET"]}')
            detection_result = self.controller.detect(frame)
            detection_result2 = self.controller2.detect(frame)
            detection_result3 = self.controller3.detect(frame)
            detection_result = self.voting_on_detections(detection_result, detection_result2, detection_result3)
            #############################################################
            print(f'[+] Detected Result #{idx+1} : AF-{detection_result["AF"]} / EF-{detection_result["EF"]} / AT-{detection_result["AT"]} / ET-{detection_result["ET"]}')
            ...
            candidates.append((detection_result["AF"], detection_result["EF"]))
